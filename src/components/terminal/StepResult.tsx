...
export function StepResult({ result, onReset }: StepResultProps) {
  ...
  return (
    <div className="flex flex-col items-center py-6 space-y-5">
      ...
      {/* Imprimir nota fiscal — apenas em pagamentos aprovados */}
      {isSuccess && (
        <div className={`w-full transition-all duration-500 delay-500 ${visible ? "opacity-100 translate-y-0" : "opacity-0 translate-y-3"}`}>
          <button
            onClick={() => setPaperError(true)}
            className="w-full py-3 px-4 rounded-xl border-2 border-gray-200 text-gray-600 font-semibold text-sm hover:border-gray-300 hover:bg-gray-50 transition-all duration-200 flex items-center justify-center gap-2"
          >
            <svg className="w-4 h-4 flex-shrink-0" fill="none" viewBox="0 0 24 24">
              ...
              <rect x="3" y="9" width="18" height="9" rx="2" stroke="currentColor" strokeWidth="1.8" />
              <path d="M6 18v3h12v-3" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round" />
              <path d="M7 13h.01M7 16h5" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" />
            </svg>
          ...
          {paperError && (
            <div className="mt-2 flex items-start gap-2 px-3 py-2.5 bg-amber-50 border border-amber-200 rounded-xl">
              ...
              <div>
                <p className="text-xs font-bold text-amber-700">Erro de impressão</p>
                <p className="text-xs text-amber-600 mt-0.5">Bobina sem papel. Substitua o rolo e tente novamente.</p>
              </div>
            </div>
          )}
        </div>
